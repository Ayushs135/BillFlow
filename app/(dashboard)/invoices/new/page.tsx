 ...
 * @description Interactive Invoice Builder Page
 * 
 * Features:
 * - Dynamic line items editor (add, remove, edit rows with real-time Decimal arithmetic preview).
 * - Automatic pre-fill of sequential invoice number.
 * - Client dropdown selector scoped to user's directory.
 * - Fixed-percentage tax & discount bounds validation.
 * - "Save as Draft" and "Create & Mark Sent" workflows.
 */
...
export default function NewInvoicePage() {
  ...
  // Form State
  const now = new Date();
  const defaultIssueDate = now.toISOString().split('T')[0];
  const fourteenDaysAhead = new Date(now.getTime() + 14 * 24 * 60 * 60 * 1000);
  const defaultDueDate = fourteenDaysAhead.toISOString().split('T')[0];
  ...
  // Load initial clients, settings, and next invoice number
  useEffect(() => {
    let ignore = false;

    async function loadData() {
      try {
        const [clientsRes, settingsRes, nextNumRes] = await Promise.all([
          fetch('/api/clients'),
          fetch('/api/settings'),
          fetch('/api/invoices/next-number'),
        ]);

        const [clientsData, settingsData, nextNumData] = await Promise.all([
          clientsRes.json(),
          settingsRes.json(),
          nextNumRes.json(),
        ]);
        ...
        if (!ignore) {
          setError('Failed to load initial invoice settings.');
        }
      } finally {
        if (!ignore) {
  ...
  // Real-time monetary calculations
  ...
  // Form submission
  const handleSubmit = async (e: React.FormEvent, submissionStatus: 'DRAFT' | 'SENT') => {
    e.preventDefault();
    setError(null);
  ...
  if (loadingInitial) {
    return (
      <div className="max-w-4xl mx-auto p-12 text-center bg-white rounded-2xl border border-slate-200 shadow-xs">
        ...
        <p className="text-sm font-medium text-slate-600">Preparing invoice form...</p>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6 pb-12">
      ...
      <div>
        ...
        <p className="text-slate-500 text-sm mt-1">
          Fill out billing details and itemize products or services.
        </p>
      </div>
      ...
      {clients.length === 0 ? (
        <div className="bg-white p-8 rounded-2xl border border-slate-200 shadow-xs text-center space-y-4">
          ...
          <div>
            <h2 className="text-lg font-bold text-slate-900">No Clients Found</h2>
            <p className="text-sm text-slate-500 max-w-md mx-auto mt-1">
              You need at least one client before creating an invoice. Add your first client to get started.
            </p>
          </div>
          <Link
            href="/clients/new"
        ...
        <form onSubmit={(e) => handleSubmit(e, 'DRAFT')} className="space-y-6">
          ...
          <div className="bg-white rounded-2xl border border-slate-200 shadow-xs p-6 sm:p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">Auto-sequenced based on settings prefix.</p>
              </div>
            ...
            <div className="md:col-span-7 bg-white rounded-2xl border border-slate-200 shadow-xs p-6 space-y-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  Optional notes or payment instructions displayed on the bottom of the invoice.
                </p>
              </div>
            </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-end gap-3 pt-4">
            <Link
              href="/invoices"
              className="w-full sm:w-auto px-5 py-2.5 rounded-lg border border-slate-300 text-slate-700 text-sm font-medium hover:bg-slate-50 transition text-center"
            >
              Cancel
            </Link>
            <button
              ...
              className="w-full sm:w-auto px-5 py-2.5 rounded-lg border border-slate-300 bg-white text-slate-700 text-sm font-semibold hover:bg-slate-50 disabled:opacity-60 transition cursor-pointer"
            >
            ...
            <button
              ...
              className="w-full sm:w-auto flex items-center justify-center gap-2 px-6 py-2.5 rounded-lg bg-blue-600 text-white text-sm font-semibold hover:bg-blue-700 disabled:opacity-60 transition cursor-pointer shadow-sm"
            >
              {saving ? (
                <>
